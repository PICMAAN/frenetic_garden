...
public class InventarioUI : MonoBehaviour
{
    ...
    private void CrearSlots()
    {
        ...
        foreach (Ingrediente ing in InventarioManager.Instancia.ingredientesDisponibles)
        {
    ...
    {
        List<Ingrediente> lista = InventarioManager.Instancia.ingredientesDisponibles;

        for (int i = 0; i < slotsCreados.Count && i < lista.Count; i++)
        {
            Ingrediente ing = lista[i];
